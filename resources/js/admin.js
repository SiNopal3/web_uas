...
(function () {
    ...
    function initCharts() {
        ...
        if (ctxPie) {
            charts.pie = new Chart(ctxPie, {
                type: 'doughnut',
                data: {
                    labels: ['Administrator', 'Manager', 'Analyst', 'Operator', 'Viewer'],
                    datasets: [{
                        data: [2, 4, 8, 12, 18],
                        backgroundColor: ['#e0b472', '#38bdf8', '#22c55e', '#a855f7', '#64748b'],
                        borderWidth: 1,
                        borderColor: '#0f1218'
                    }]
                },
    ...
    function renderUsersTable(paginatedData) {
        ...
        tbody.innerHTML = paginatedData.data.map(user => {
            ...
            const roleName = user.role?.name || (user.roles && user.roles.length > 0 ? user.roles[0].name : 'Viewer');
            const statusBadge = user.status === 'active' 
                ? '<span class="badge bg-success bg-opacity-25 text-success border border-success px-3 py-1">ACTIVE</span>' 
                : `<span class="badge bg-secondary text-white px-3 py-1">${(user.status || 'active').toUpperCase()}</span>`;
            ...
            return `
                <tr>
                    <td class="px-3 fw-bold text-muted">#${user.id}</td>
                    <td>
